...
public class PaintPane extends BorderPane {
		...
		canvas.setOnMouseDragged(event -> {
			if(selectionButton.isSelected()) {
				...
				if(selectedFigure instanceof Rectangle) {
					...
					rectangle.getTopLeft().addX(diffX);
					rectangle.getBottomRight().addX(diffX);
					rectangle.getTopLeft().addY(diffY);
					rectangle.getBottomRight().addY(diffY);
				} else if(selectedFigure instanceof Ellipse) {
					Ellipse ellipse = (Ellipse) selectedFigure;
					ellipse.getCenterPoint().addX(diffX);
					ellipse.getCenterPoint().addY(diffY);
				}
				redrawCanvas();
			}
		});
	...
	private void changeShadow() {
		if(selectionButton.isSelected() && selectedFigure != null) {
			selectedFigure.setShadowType(getShadowType());
		}
		redrawCanvas();
	}
	private void changeBiselado() {
		if(selectionButton.isSelected() && selectedFigure != null) {
			selectedFigure.setBiselado(biseladoCheckBox.isSelected());
		}
		redrawCanvas();
